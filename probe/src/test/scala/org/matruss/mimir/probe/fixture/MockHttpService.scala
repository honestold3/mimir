...
class MockHttpService extends Actor
{
	...
	def receive:Receive =
	{
		...
		case HttpRequest(HttpMethods.GET,"/content&",_,_,_) => sender ! rspBadRequest
		case HttpRequest(HttpMethods.GET,"/content",_,_,_) => sender ! rspNotFound
		case HttpRequest(HttpMethods.GET,"/content/money",_,_,_) => sender ! rspForbidden
		case HttpRequest(HttpMethods.GET,"/nocontent",_,_,_) => sender ! rspFilure
		case x => sender ! Status.Failure(new IOException)
	}
}
...
object MockHttpService
{
	...
	val rspFilure = HttpResponse( status = 500, entity = "I am dead")
